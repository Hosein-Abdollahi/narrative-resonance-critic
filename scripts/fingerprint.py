"""
fingerprint.py — the "emotional fingerprint" of a text.

Renders a whole text as a horizontal strip of vertical color stripes, one per
chunk, reading left→right like a timeline. Each stripe's HUE comes from the
chunk's dominant emotion; its GLOW (opacity) comes from the chunk's intensity —
so emotional peaks light up and flat passages fade. Binning by column count
means the same code handles a paragraph or a novel.

Depends only on the public engine API.
"""
...
# Stripes never fully disappear — a flat passage still reads as a faint band.
_MIN_GLOW = 0.18


def _aggregate(emotion_dicts: List[Dict[str, float]]) -> Dict[str, float]:
    """Mean emotion vector across a bin of sentences."""
...
def build_fingerprint(
    ...
    """
    Analyze `text` and reduce it to at most `max_columns` columns.

    Each column dict: {color, intensity, valence, arousal, dominant, span}.
    `span` is the (start, end) sentence indices the column covers.
    ...
    for b in bins:
        ...
        # Peak-leaning intensity so a bright moment in a bin still glows after
        # binning, rather than being averaged into the dark.
...
def fingerprint_svg(
    ...
    """Render columns as a single SVG strip. Opacity tracks intensity (glow)."""
    if not columns:
        return ""
...
def text_to_fingerprint_svg(
    ...
    """Convenience: text → fingerprint SVG (or None if nothing to render)."""
