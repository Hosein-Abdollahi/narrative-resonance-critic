...
سه سطح تحلیل:
  fast     → weighted-lexicon (بدون نیاز به هیچ چیز اضافه)
  balanced → transformer محلی (نیاز به: pip install transformers torch)
  precise  → Claude API     (نیاز به: ANTHROPIC_API_KEY)

This module is a library — it never prints to stdout and loads the
transformer lazily on first balanced-mode use.

Accuracy model (fast mode):
  score(emotion) = Σ  weight(word) × modifier(context)
  where weight   ∈ (0, 1]  encodes how strongly a word signals the emotion,
  and   modifier scales by nearby boosters ("extremely"), diminishers
  ("slightly"), and negation ("not", "without", "wasn't" → suppressed).
"""
...
# Returned as the dominant when a text carries no real emotional signal.
NEUTRAL = "neutral"
NEUTRAL_COLOR = "#6b6b70"
# A text is "neutral" when its strongest emotion's absolute magnitude is below
# this floor. Tunable: raise it if faint residuals still get an emotion label,
# lower it if genuinely mild emotions get flattened to neutral.
_NEUTRAL_FLOOR = 0.15
...
# ─────────────────────────────────────────────
# Weighted lexicon  {emotion: {word_or_phrase: signal_weight}}
#   weight ≈ how strongly the term signals the emotion (0.3 mild … 1.0 intense)
#   Key inflections are listed explicitly so recall doesn't depend on a stemmer.
# ─────────────────────────────────────────────
...
# Words that flip / cancel the following emotion term within a small window.
_NEGATION_WORDS = {
    "not", "no", "never", "neither", "nor", "none", "nothing", "without",
    "hardly", "barely", "scarcely",
}

# Multiplicative context modifiers (applied to the weight of a nearby keyword).
_BOOSTERS = {
...
# "neutral" AND "disgust" are intentionally UNMAPPED. The model's disgust class
# is a hair-trigger — it tags flat descriptive prose ("blank pages", "he sat in
# his chair") as disgust — so its label is too noisy to trust. disgust is real,
# but sourced from the lexicon (overlay) like hope/awe, not from the model.
# fear→fear (acute) splits from anxiety (chronic worry), so anxiety is overlay-driven.
_TRANSFORMER_LABELS = ["anger", "disgust", "fear", "joy", "neutral", "sadness", "surprise"]
# Emotions the transformer can't supply (no usable label, or too noisy) → overlay:
#   nostalgia, tenderness, calm, anxiety, hope, awe, disgust.
# Emotions the transformer under-reads → overlay rescue: melancholy, wonder.
...
def transformers_available() -> bool:
    """
    Cheaply report whether balanced mode's dependencies are importable.
    Does NOT import, load, or download the model — safe to call at UI build time.
    """
...
def _ensure_model() -> bool:
    """Load the transformer once, on first use; cache success or failure."""
...
_SENT_SENTINEL = "\u0000"  # temporary stand-in for protected periods
...
def _expand_contractions(text: str) -> str:
    ...
    text = re.sub(r"n['’]t\b", " not", text, flags=re.IGNORECASE)  # isn't, didn't, ...
    return text


def _split_sentences(text: str) -> List[str]:
    """Abbreviation- and decimal-aware sentence splitter (dependency-free)."""
    protected = text
    # Dotted abbreviations like a.m., p.m., i.e., e.g., U.S. — protect inner dots.
...
def _normalize_candidates(word: str):
    """Yield the word plus conservative inflection variants for lexicon lookup."""
    yield word
    if len(word) > 3 and word.endswith("s") and not word.endswith("ss"):
        yield word[:-1]                       # loves → love, fears → fear
    if len(word) > 4 and word.endswith("ies"):
        yield word[:-3] + "y"                 # mysteries → mystery, worries → worry
    if len(word) > 4 and word.endswith("ied"):
        yield word[:-3] + "y"                 # cried → cry, worried → worry
    if len(word) > 4 and word.endswith("ed"):
        yield word[:-2]                       # laughed → laugh, mourned → mourn
        yield word[:-1]                       # loved → love, adored → adore
    if len(word) > 5 and word.endswith("ing"):
        yield word[:-3]                       # laughing → laugh
        yield word[:-3] + "e"                 # adoring → adore, caring → care


def _lexicon_hit(word: str):
    """Return (emotion, weight) for a word, trying conservative inflections."""
...
def _negated_emotions(text: str) -> set:
    """
    Emotions whose lexicon words appear ONLY negated ("no fear", "not happy").
    Used to correct the transformer, which is blind to negation.
    """
...
# ─────────────────────────────────────────────
# Derived signals (the primitives Phase-2 visuals stand on)
#   valence   : negative ↔ positive     (-1 … +1)
#   arousal   : low-energy ↔ high-energy ( 0 … 1)
#   intensity : absolute emotional charge, comparable across texts ( 0 … 1)
# ─────────────────────────────────────────────

# Activating vs deactivating emotions (the arousal axis).
...
def valence_of(emotions: Dict[str, float]) -> float:
    """Positive vs negative balance, in [-1, 1]. Nostalgia is treated as neutral."""
...
def arousal_of(emotions: Dict[str, float]) -> float:
    """Share of emotional energy that is activating (high-arousal), in [0, 1]."""
...
def _analyze_with_keywords(text: str, _return_raw: bool = False):
    """
    Weighted-lexicon analysis with negation + intensity modifiers.
    'not angry' → suppressed; 'extremely angry' → boosted; 'slightly' → reduced.

    If _return_raw is True, also returns the pre-normalization score dict, whose
    magnitude is comparable ACROSS texts (the normalized profile is not).
    """
    lowered = _expand_contractions(text.lower())
    words = re.findall(r"\b\w+\b", lowered)
    scores = {e: 0.0 for e in EMOTIONS}

    # Single-word terms (exact, then conservative inflection fallback).
    for idx, word in enumerate(words):
        hit = _lexicon_hit(word)
        if hit is None:
            continue
...
def _analyze_with_transformer(text: str) -> Dict[str, float]:
    ...
    try:
        import torch
        cap = 400  # guard against pathological inputs
        ...
        # Overlay lexicon evidence — but ONLY when real keywords exist, so the
        # floor profile {all 0.1} can't leak a phantom dominant onto emotionless
        # sentences (this is what produced "nostalgia 9%" on a disgust line).
        ...
        # The transformer ignores negation; suppress emotions that appear only
        # negated ("no fear", "not happy") so they don't fire on the model's read.
...
def _analyze_full(text: str, mode: str = "balanced") -> Tuple[Dict[str, float], float]:
    """
    Return (normalized_emotions, magnitude).

    `magnitude` is the peak strength of the strongest emotion on an ABSOLUTE
    scale — comparable across texts — taken before any per-call max-normalization.
    This is what makes a cross-sentence intensity curve possible.
    """
    if not text or len(text.strip()) < 3:
        return {e: 0.1 for e in EMOTIONS}, 0.0

    if mode == "precise" and os.environ.get("ANTHROPIC_API_KEY"):
        profile = _analyze_with_api(text)            # API values are absolute
        return profile, max(profile.values())
    if mode == "fast":
        profile, raw = _analyze_with_keywords(text, _return_raw=True)
        return profile, max(raw.values())            # raw, pre-normalization
    profile = _analyze_with_transformer(text)        # already absolute (not max-normalized)
    return profile, max(profile.values())


def analyze(text: str, mode: str = "balanced") -> dict:
    """
    Rich single-text analysis: the emotion profile plus the three derived
    primitives (intensity, valence, arousal) and the dominant emotion.
    """
...
def analyze_emotions_by_sentence(text: str, mode: str = "balanced") -> List[Dict]:
    ...
    Each item carries the per-sentence emotion profile plus three derived
    signals: `intensity` (0–1, scaled so the most charged sentence in THIS text
    is 1.0), `valence` (-1..1), and `arousal` (0..1). Existing keys are unchanged.
    """
    sentences = [s for s in _split_sentences(text) if len(s) > 5]
    if not sentences:
        return []

    # First pass: profile + absolute magnitude for every sentence.
    raw = [(sent, *_analyze_full(sent, mode)) for sent in sentences]

    # Robust document scale: the 92nd-percentile magnitude, so a single freak
    # sentence can't wash the whole curve into darkness. (Max for short texts.)
    ...
    for sent, emotions, magnitude in raw:
        ...
        results.append({
            ...
            "color":          color,
            "intensity":      round(intensity, 4),            # doc-relative, absolute
...
def shade_sentences_html(text: str, sentence_analysis: List[Dict]) -> str:
    """
    Tint every sentence by its dominant emotion, with opacity tracking
    intensity. Unlike keyword highlighting, this lights up on imagery and
    subtext too, because every sentence carries an emotion + intensity score.

    Walks the original text so spacing, punctuation, quotes, and paragraph
    breaks are preserved exactly. Returns ready-to-render HTML.
    """
    if not sentence_analysis:
